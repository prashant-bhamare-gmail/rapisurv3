...
@Component({
    selector: 'app-financial-plan-report',
    templateUrl: './financial-plan-report.component.html',
    styleUrls: ['./financial-plan-report.component.scss'],
    providers: [NumberTransformPipe]
})
export class FinancialPlanReportComponent implements OnInit {
    financialPlanReport: any[] = [];
    ...
    getFinancialPlanData() {
        this.dashboardService.getProjectFinancialPlanReport(this.projectId).subscribe((res: any) => {
            if (res.status == HttpStatusCode.Ok) {
                this.financialPlanReport = res.data
                this.summaryData = res.summation
                this.summaryDataFilter = { ...this.summaryData }
                this.financialPlanReport.forEach(item => {
